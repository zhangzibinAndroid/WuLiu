...
public class DrivingActivity extends BaseActivity {
    ...
    @BindView(R.id.img_backDriving)
    ImageView imgBackDriving;
    ...
    private void initView() {
        ...
        Bitmap bitmap = intent.getParcelableExtra("DrivingPic");
        imgDriving.setImageBitmap(bitmap);
    }

    @OnClick({R.id.img_backDriving, R.id.img_driving})
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.img_backDriving:
                sendBitmap();
                break;
    ...
    private void sendBitmap(){
        Bitmap bitmap = ((BitmapDrawable)imgDriving.getDrawable()).getBitmap();
        Intent intent = new Intent();
        intent.putExtra("FromDrivingPic",bitmap);
        setResult(10, intent);
        finish();
    }
                ...
                switch (which) {
                    ...
                    case TAKE_PICTURE: // 拍照
                        ...
                        tempUri = Uri.fromFile(new File(Environment
                                .getExternalStorageDirectory()+"/Driving.jpg"));
        ...
        if (resultCode == RESULT_OK) { // 如果返回码是可以用的
            switch (requestCode) {
                ...
                case CROP_SMALL_PICTURE:
                    if (data != null) {
                        setImageToView(data,imgDriving); // 让刚才选择裁剪得到的图片显示在界面上
                    }
                    break;
            }
        }
